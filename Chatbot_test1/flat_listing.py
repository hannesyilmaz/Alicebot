...
def main():
    ...
    def flat_listing():
        flat_list = []
        _ = [flat_list.extend(item) if isinstance(item, list) else flat_list.append(item) for item in return_find_sentence() if item]
        return flat_list

    def word_toke_tag():
        word_token = [nltk.word_tokenize(item) for item in flat_listing()]
        #print(word_token)
        #zlist = []
